...
import '../../core/wigets/cousstom_student_app_Bar.dart';
...
class StudentsDataSecondSecondaryScreen extends StatefulWidget {
  const StudentsDataSecondSecondaryScreen({super.key});

  @override
  State<StudentsDataSecondSecondaryScreen> createState() =>
      _StudentsDataSecondSecondaryScreenState();
}

class _StudentsDataSecondSecondaryScreenState
    extends State<StudentsDataSecondSecondaryScreen> {
                  ...
                  children: [
                    Padding(
                      ...
                      child: CousstomStudentAppBar(
                        tittel: " بيانات الطلاب في الصف الثاني الثانوي👥",
                      ),
                    ),
                            ...
                            builder: (context, state) {
                              if (state is StudentsLoading) {
                                ...
                                final students = state.students
                                  ..sort((a, b) => b.createdAt!.compareTo(a.createdAt!)); // الجديد يظهر أول

                                List<StudentModel> displayedStudents;
                                if (searchController.text.isEmpty) {
                                  displayedStudents = students; // كل الطلاب لو البحث فاضي
                                } else {
                                  final query = searchController.text.toLowerCase();
                                  displayedStudents = students.where((student) {
                                    return student.name.toLowerCase().contains(query) ||
                                        student.phone.toLowerCase().contains(query) ||
                                        student.city.toLowerCase().contains(query);
                                  }).toList();
                                }
